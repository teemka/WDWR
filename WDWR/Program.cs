        ...
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                for (int j = 0; j < 3; j++) // Ograniczenia na możliwości rafinacji
                {
                    ...
                    oilBuy[i][j] = cplex.NumVar(0, 1000);
                }
            }
            ILinearNumExpr mean = cplex.LinearNumExpr();
            for (int i = 0; i < 2; i++)
            {
                cplex.Ge(cplex.ScalProd(hardness3, oilProduce[i]), cplex.ScalProd(hardness, oilProduce[i])); // Hardness 
                cplex.Le(cplex.ScalProd(hardness, oilProduce[i]),cplex.ScalProd(hardness6, oilProduce[i]));
            }
            for (int i = 0; i < 3; i++)
            {
            ...
            cplex.AddMaximize(cplex.Diff(cplex.Diff(cplex.Sum(cplex.ScalProd(price,oilProduce[0]), cplex.ScalProd(price,oilProduce[1])),cplex.Sum(cplex.ScalProd(priceStorage,oilStore[0]), cplex.ScalProd(priceStorage,oilStore[1]), cplex.ScalProd(priceStorage, oilStore[2]))),
                cplex.Sum(cplex.ScalProd(oilCost[0], oilProduce[0]), cplex.ScalProd(oilCost[1], oilProduce[1]))));
